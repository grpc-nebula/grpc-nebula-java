...
public final class NettyChannelBuilder
   ...
   *
   * 这个值要和AbstractManagedChannelImplBuilder#IDLE_MODE_MAX_TIMEOUT_DAYS保持一致
   *
   * @author sxp
   * @sicne 2019/2/25
   */
   ...
   *
   * @since 1.3.0
   * @since 2019-02-25 modify by sxp 如果enable为true,禁用客户端空闲30分钟自动关闭客户端连接的特性
   */
  @Override
  public NettyChannelBuilder keepAliveWithoutCalls(boolean enable) {
    keepAliveWithoutCalls = enable;
    if (enable) {
      // 如果在连接上没有发生服务调用时允许发送保持活动的HTTP/2 ping，则禁用客户端空闲30分钟自动关闭客户端连接的特性
      idleTimeout(IDLE_MODE_MAX_TIMEOUT_DAYS, TimeUnit.DAYS);
    }
    return this;
  }
